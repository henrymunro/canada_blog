...
*/
  function getEntries (req, res, next, clause = {endDate: null}) {
    logger.debug('Request recieved to get entries', {loggerModule, URL: req.originalUrl})
...
*/
  ...
  function _performMultipleDatabaseUpdate (req, res, updates) {
      ...
      return new Promise((resolve, reject) => {
        Schema.findById({_id: update._id}, (err, entry) => {
          if (err) {
            ...
            env ? reject(err) : reject({error: true, msg: 'An error has occoured'}) // Only send stack in dev
          }
          Object.assign(entry, update).save((err, entry) => {
            if (err) {
              logError(err, `Error updating entry by id  ${req.originalUrl}`, loggerModule)
    ...
    Promise.all(updatesPromise).then((result) => {
      ...
      env ? res.send(err) : res.send({error: true, msg: 'An error has occoured'})
    })
  }
