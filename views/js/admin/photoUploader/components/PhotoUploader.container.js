...
export default class PhotoUploader extends React.Component {
  ...
  uploadPhotos () {
    ...
    mapLimit(this.props.uploadedPhotos, 5, (photo, callback) => {
      // Promise((resolve, reject) => {
      console.log('Uploading photo: ', photo._id)
      ...
      this.props.uploadPhotoToServer(photo)  // UPLOAD_PHOTO_TO_SERVER
          .then((res) => {
            ...
            console.log('Saved photo to DB: ', photo._id)
            // Upload the photo to the S3 bucket
            return this.uploadPhotoToS3(({signedRequest, photo, databaseInfo, _id: photo._id}))
          }).then(response => {
            console.log('Calling callback: ', photo._id)
  ...
  uploadPhotoToS3 ({signedRequest, photo, databaseInfo, _id}) {
    // Upload the photo to the S3 bucket
    return new Promise((resolve, reject) => {
      console.log('Saving photo to S3')
      this.props.updateUploadedPhoto('uploading', true, _id)
      this.props.uploadPhotoToS3({signedRequest, photo, databaseInfo})
        .then(result => {
          ...
          this.props.updateUploadedPhoto('uploading', false, _id)
          this.props.updateUploadedPhoto('uploaded', true, _id)
          ...
          this.props.updateUploadedPhoto('uploading', false, _id)
          this.props.updateUploadedPhoto('uploaded', false, _id)
          this.props.updateUploadedPhoto('error', true, _id)
